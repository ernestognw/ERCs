...
author: Henry Arneson (@godzillaba), Chris Buckland (@yahgwai), Ernesto García (@ernestognw)
...
### ERC-7786 Compatibility

Implementations MAY choose to implement [ERC-7786] gateway interfaces to provide standardized cross-chain messaging compatibility. This enables ERC-7888 systems to integrate with existing ERC-7786 infrastructure and tooling.

#### Operation Attributes

This specification defines the following optional operation attributes for ERC-7786 compatibility:

##### `verifyBroadcast(bytes32,address,string)`

ERC-7786 implementations MAY support broadcast message verification using this attribute:

```solidity
abi.encodeWithSignature("verifyBroadcast(bytes32,address,string)", message, publisher, sourceChain);
```

Where:
- `message`: The 32-byte message to verify
- `publisher`: The address that published the message  
- `sourceChain`: The source chain identifier (for context/validation)

When this attribute is present, receivers SHOULD decode the message payload accordingly and perform broadcast message verification using the provided storage proof attributes.

##### `updateProver(address)`

ERC-7786 implementations MAY support block hash prover updates using this attribute:

```solidity
abi.encodeWithSignature("updateProver(address)", bhp);
```

Where:
- `bhp`: The address of the IBlockHashProver implementation to update

When this attribute is present, receivers SHOULD decode the message payload as the prover implementation data and perform the prover update using the provided storage proof attributes.

These operation attributes enable ERC-7888 functionality within standard ERC-7786 message processing while maintaining backwards compatibility with existing implementations.

#### Broadcaster as ERC-7786 Gateway Source

A Broadcaster MAY implement `IERC7786GatewaySource` to enable standard messaging:

```solidity
interface IERC7888BroadcasterWithERC7786 is IBroadcaster, IERC7786GatewaySource {
    /// @notice Sends a cross-chain message using ERC-7786 interface
    /// @param chainId Target chain identifier (unused for broadcast)
    /// @param receiver Receiver address (empty string "" for broadcast semantics)
    /// @param data Message data (MUST be exactly 32 bytes for ERC-7888 compatibility)
    /// @param attributes Optional attributes (unused in basic implementation)
...
#### Receiver as ERC-7786 Gateway Destination

A Receiver MAY implement `IERC7786Receiver` for standardized message processing using the operation attributes defined above:

```solidity
interface IERC7888ReceiverWithERC7786 is IReceiver, IERC7786Receiver {
    ...
    /// @param payload Message payload containing operation-specific data
    /// @param attributes Operation and storage proof attributes:
    ///        - verifyBroadcast(bytes32,address,string): Message verification operation
    ///        - updateProver(address): Prover update operation  
    ///        - route((address,bytes,uint256)[]): Route with proofs and versions
    ///        - storageProof(bytes): Storage proof data
    ///        - targetBlock(uint256): Optional target block number
    function executeMessage(
        ...
        (bytes32 operation, bytes32 message, address publisher) = _parseOperationAttributes(attributes);
        
        if (operation == keccak256("verifyBroadcast(bytes32,address,string)")) {
            // Parse storage proof attributes
            (address[] memory route, bytes[] memory bhpInputs, bytes memory storageProof) = 
                _parseStorageProofAttributes(attributes);
            
            RemoteReadArgs memory readArgs = RemoteReadArgs({
                route: route,
                bhpInputs: bhpInputs,
                storageProof: storageProof
            });
            
            verifyBroadcastMessage(readArgs, message, publisher);
        } else if (operation == keccak256("updateProver(address)")) {
            // Decode prover address from attribute
            address bhpCopyAddress = _parseProverUpdateAttribute(attributes);
            // Payload contains additional prover data if needed
            _handleProverUpdate(attributes, bhpCopyAddress, payload);
        }
        
        return IERC7786Receiver.executeMessage.selector;
    }
    
    /// @notice Parse operation attributes to determine the operation type and parameters
    function _parseOperationAttributes(bytes[] calldata attributes) 
        internal pure returns (bytes32 operation, bytes32 message, address publisher) {
        for (uint256 i = 0; i < attributes.length; i++) {
            bytes4 selector = bytes4(attributes[i][:4]);
            
            if (selector == bytes4(keccak256("verifyBroadcast(bytes32,address,string)"))) {
                operation = keccak256("verifyBroadcast(bytes32,address,string)");
                (message, publisher,) = abi.decode(attributes[i][4:], (bytes32, address, string));
                break;
            } else if (selector == bytes4(keccak256("updateProver(address)"))) {
                operation = keccak256("updateProver(address)");
                break;
            }
        }
    }
...
### Benefits of ERC-7786 Integration

By implementing ERC-7786 compatibility, ERC-7888 systems can integrate with future cross-chain messaging infrastructure and standards. This optional integration enables standardized storage proof attributes and cross-protocol communication while maintaining the core ERC-7888 functionality.

### Operation Attributes

Operation attributes are used to identify the operation to be performed by the Receiver. This is useful for ERC-7786 compatibility.
