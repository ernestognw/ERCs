...
A Receiver MAY implement `IERC7786GatewayDestination` for message processing:

```solidity
interface IERC7888ReceiverWithERC7786 is IReceiver, IERC7786GatewayDestination {
    ...
    /// @param attributes Storage proof attributes following ERC-7889 format:
    ...
    function processMessage(
        ...
        require(data.length == 32); // Message must be 32 bytes
        require(attributes.length >= 3); // Insufficient attributes
        
        // Parse ERC-7889 storage proof attributes
        (address[] memory route, bytes[] memory bhpInputs, bytes memory storageProof) = 
            abi.decode(attributes[0], (address[], bytes[], bytes));
        ...
        bytes32 message = bytes32(data);
        verifyBroadcastMessage(readArgs, message, sender);
    }
}
```
