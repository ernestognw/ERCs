...
To support [ERC-7786] messaging, it is RECOMMENDED that the `payload` of the `sendMessage` function is encoded as follows:

```solidity
abi.encodeWithSelector(IReceiver.verifyBroadcastMessage.selector, broadcasterReadArgs, message, publisher);
```

If the `payload` is encoded as above, the gateway SHOULD route the message to the `IReceiver::verifyBroadcastMessage` function.

```solidity
abi.encodeWithSelector(IReceiver.updateBlockHashProverCopy.selector, bhpPointerReadArgs, bhp);
```

If the `payload` is encoded as above, the gateway SHOULD route the message to the `IReceiver::updateBlockHashProverCopy` function.
...
```solidity
interface IERC7786Broadcaster is IBroadcaster, IERC7786GatewaySource {
...
A Receiver MAY implement `IERC7786Receiver` for standardized message processing:

```solidity
interface IERC7786BroadcasterReceiver is IReceiver, IERC7786Receiver {
    ...
    function executeMessage(
        ...
        if(selector == IReceiver.verifyBroadcastMessage.selector) {
            (RemoteReadArgs memory broadcasterReadArgs, bytes32 message, address publisher) = abi.decode(payload[0][4:], (RemoteReadArgs, bytes32, address));
            ...
            (RemoteReadArgs memory bhpPointerReadArgs, IBlockHashProver bhp) = abi.decode(payload[0][4:], (RemoteReadArgs, IBlockHashProver));
...
By recommending a standard way to encode the `payload` of the `sendMessage` function, ERC-7888 systems can integrate with existing ERC-7786 infrastructure and tooling. This enables ERC-7888 systems to integrate with existing ERC-7786 infrastructure and tooling.
